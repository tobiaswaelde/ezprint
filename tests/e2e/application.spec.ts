...
async function expectStandardToolbars(toolbars: Locator) {
  await expect(toolbars).not.toHaveCount(0);
  const count = await toolbars.count();
  for (let index = 0; index < count; index++) {
    await expect(toolbars.nth(index)).toHaveCSS('min-height', '49px');
  }
}
...
test('setup, navigation, persistence, accessibility, and responsive shell', async ({ page }) => {
  ...
  await expectStandardToolbars(printsToolbar.locator(':scope > div > [data-slot="root"]'));
  ...
  await expectStandardToolbars(tableToolbar);
  ...
  const customerSelect = createPrintDialog.getByRole('button', { name: 'Kunden', exact: true });
  ...
  await expect(customerSelect).toContainText('Acme Updated');
  await createPrintDialog.getByLabel('Name', { exact: true }).fill('Unsaved parent print');
  await expect(createPrintDialog.getByLabel('Name', { exact: true })).toHaveValue('Unsaved parent print');
  await createPrintDialog.getByRole('button', { name: 'Kunden erstellen', exact: true }).click();
  const inlineCustomerDialog = page.getByRole('dialog', { name: 'Erstellen · Kunden', exact: true });
  await inlineCustomerDialog.getByLabel('Name', { exact: true }).fill('Discarded customer');
  await inlineCustomerDialog.getByRole('button', { name: 'Abbrechen' }).click();
  await expect(customerSelect).toContainText('Acme Updated');
  await createPrintDialog.getByRole('button', { name: 'Kunden erstellen', exact: true }).click();
  await inlineCustomerDialog.getByLabel('Name', { exact: true }).fill('Inline customer');
  await inlineCustomerDialog.getByRole('button', { name: 'Speichern' }).click();
  await expect(inlineCustomerDialog).toBeHidden();
  await expect(customerSelect).toContainText('Inline customer');
  await expect(createPrintDialog.getByLabel('Name', { exact: true })).toHaveValue('Unsaved parent print');
  await createPrintDialog.getByRole('button', { name: 'Abbrechen' }).click();
  ...
  await expectStandardToolbars(settingsToolbar);
  expect((await settingsToolbar.boundingBox())!.height).toBeGreaterThanOrEqual(49);
  await page.getByRole('switch', { name: 'Druckserien' }).click();
  await page.getByRole('switch', { name: 'Spulenverwaltung' }).click();
  await page.getByRole('button', { name: 'Speichern', exact: true }).click();
